...
| Daemon survives close | TODO | Attach `scripts/smoke-installed-app.sh --dry-run` and `scripts/smoke-installed-app.sh` output proving installed daemon `/internal/health`, settings persistence, inference mode persistence, and installed folder source discovery/queueing via the structured `installed_app_smoke` line; then close main window, confirm hotkey/indexing continue, reopen with state intact. |
