<div align="center">
  ...
  <a href="https://cerul.ai">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="apps/desktop/public/brand/icon-paper/cerul-icon-paper-256.png" />
      <img src="apps/desktop/public/brand/icon-graphite/cerul-icon-graphite-256.png" alt="Cerul" width="96" />
    </picture>
  </a>
  <h1>Cerul App</h1>
  <p><strong>让视频有出处。</strong></p>
  <p>别拖进度条了，直接搜——本地优先、由你掌控。</p>
  ...
  <p>
    <a href="https://app.cerul.ai"><strong>获取应用</strong></a> &middot;
    ...
    <a href="#社区"><img src="https://img.shields.io/badge/%E5%8A%A0%E5%85%A5-%E5%BE%AE%E4%BF%A1%E7%BE%A4-07C160?style=flat-square&logo=wechat&logoColor=white" alt="加入微信群" /></a>
  </p>
  ...
  <p>
    ...
    <img alt="Status" src="https://img.shields.io/badge/status-v0.0.61-22c55e?style=flat-square" />
  </p>
  ...
  <a href="https://app.cerul.ai"><img src="apps/desktop/public/brand/screenshots/app-cerul-home.png" alt="Cerul App — 在本地搜索你看过、听过的一切" width="840" /></a>
</div>

<br />

> [!NOTE]
> **初版发布。** Cerul App 是 [Cerul Cloud](https://cerul.ai) 的源码可得、可自托管配套客户端。当前版本：**0.0.61**。核心已可运行 —— 桌面外壳、Cerul Core、索引流水线、混合搜索、浮层和托盘今天就能跑。公开安装包通过 GitHub Releases 发布；release workflow 配置 Developer ID 凭据时，macOS 制品会经过签名与公证。
...
- **按语义搜索。** 混合检索把全文检索（SQLite/FTS）与内嵌本地向量索引结合，让你找到的是那个"瞬间"，而不只是关键词。
...
5. **嵌入**：对转录分块做向量化，嵌入成功后写入内嵌 zvec 索引。
...
从源码开发时，`run.sh` 也可以读取一个本地 `.env` 文件来预置默认 provider 参数，这只是开发时的便利：
...
Cerul App 仍处于 alpha 阶段。当前版本：**0.0.61**。整条链路已端到端跑通，公开 macOS 制品现在由 release workflow 通过签名、公证和安装版冒烟作为门禁。
...
- 更完整的安装版发布冒烟覆盖
- 第三方二进制许可证审查（`ffmpeg`、`yt-dlp`）

想要更新的开箱即装构建？Star 并 Watch 本仓库 —— 公开构建会以 GitHub Release 形式发布。

## 社区

有问题、想反馈，或只是想关注进展？欢迎加入交流 —— 选择离你最近的社群即可。
...
<p align="center">
  ...
  「Cerul.ai 官方社群」<br /><br />
  <img src=".github/assets/wechat-group-qr.png" alt="Cerul.ai 官方微信社群二维码" width="240" />
</p>

**🌍 海外** —— 在 Discord 上与全球用户和贡献者交流：

<a href="https://discord.gg/qHDEMQB9vN"><img src="https://img.shields.io/badge/%E5%8A%A0%E5%85%A5-Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="加入 Discord" /></a>

> **提示：** 二维码会定期更新，可能失效。如果微信扫码失败，请通过 [Discord](https://discord.gg/qHDEMQB9vN) 或 [X / Twitter](https://x.com/cerul_hq) 联系我们获取最新入群方式。

## 它与 Cerul 的关系

Cerul App 是 [Cerul](https://github.com/cerul-ai/cerul) 平台的**源码可得、自托管**层 —— 用你自己的机器、你自己的 key 运行它。[Cerul Cloud](https://cerul.ai) 是面向团队的托管服务，提供托管式索引、视频搜索 API 和账号级同步。本应用完全可独立运行；Cloud 账号是可选的。Cerul Cloud 的账号后端**不包含**在本仓库中；桌面客户端只在你登录时调用其公开账号 API。
