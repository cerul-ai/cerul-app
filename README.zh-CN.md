...
> [!NOTE]
> **初版发布。** Cerul App 是 [Cerul Cloud](https://cerul.ai) 的源码可得、可自托管配套客户端。当前版本：**0.0.24**。核心已可运行 —— 桌面外壳、Cerul Core、索引流水线、混合搜索、浮层和托盘今天就能跑。公开安装包通过 GitHub Releases 发布；release workflow 配置 Developer ID 凭据时，macOS 制品会经过签名与公证。
...
Cerul App 仍处于 alpha 阶段。当前版本：**0.0.24**。整条链路已端到端跑通，公开 macOS 制品现在由 release workflow 通过签名、公证和安装版冒烟作为门禁。
