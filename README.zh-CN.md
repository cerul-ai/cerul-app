...
> [!NOTE]
> **Alpha 阶段。** Cerul App 是 [Cerul Cloud](https://cerul.ai) 的开源、可自托管配套客户端。当前版本：**0.0.1-alpha.2**。核心已可运行 —— 桌面外壳、本地 API、索引流水线、混合搜索、浮层和托盘今天就能跑。已签名的公开安装包仍处于发布门禁阶段；在某个 GitHub Release 明确标注"已签名/已公证"安装包之前，请先从源码构建运行。详见 [项目状态与路线图](#项目状态与路线图)。
