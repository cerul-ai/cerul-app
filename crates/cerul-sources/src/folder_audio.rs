...
const EXTENSIONS: &[&str] = &["mp3", "m4a", "flac", "ogg", "wav"];
...
impl FolderAudio {
    pub fn new(config: serde_json::Value) -> anyhow::Result<Self> {
        let path = config
            ...
            .context("folder_audio requires config.path")?;
...
#[async_trait]
impl SourcePlugin for FolderAudio {
    ...
    async fn fetch(&self, item: &DiscoveredItem) -> anyhow::Result<PathBuf> {
        let raw_path = item
            ...
            .get("raw_path")
            .and_then(|value| value.as_str())
            .context("folder_audio item is missing metadata.raw_path")?;
...
mod tests {
    ...
    #[tokio::test]
    async fn discovers_audio_files_recursively() {
        let temp = tempfile::tempdir().unwrap();
        let nested = temp.path().join("nested");
        std::fs::create_dir(&nested).unwrap();
        std::fs::write(temp.path().join("episode.mp3"), b"audio").unwrap();
        std::fs::write(nested.join("clip.WAV"), b"audio").unwrap();
        std::fs::write(temp.path().join("notes.txt"), b"not audio").unwrap();

        let source = FolderAudio::new(json!({ "path": temp.path() })).unwrap();
        let items = source.discover().await.unwrap();

        assert_eq!(items.len(), 2);
        assert!(items
            .iter()
            .any(|item| item.title.as_deref() == Some("episode")));
        assert!(items
            .iter()
    ...
    async fn fetch_returns_discovered_file_path() {
        let temp = tempfile::tempdir().unwrap();
        let audio = temp.path().join("episode.flac");
        std::fs::write(&audio, b"audio").unwrap();

        let source = FolderAudio::new(json!({ "path": temp.path() })).unwrap();
        let item = source.discover().await.unwrap().pop().unwrap();

        assert_eq!(source.fetch(&item).await.unwrap(), audio);
    }

    #[test]
    fn rejects_missing_directory() {
        let error = FolderAudio::new(json!({ "path": "/definitely/not/cerul/audio" }))
            .unwrap_err()
            .to_string();
