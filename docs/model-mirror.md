...
- File layout: `models/v1/models--<namespace>--<repo>/<revision>/files/<repo-relative-path>`
- Archive fallback layout: `models/v1/models--<namespace>--<repo>/<revision>/snapshot.tar.gz`
...
Each model is mirrored separately in its own directory. The runtime prefers the
file-level manifest so downloads can resume and retry individual files while
keeping the same snapshot layout as Hugging Face / ModelScope. The older tar.gz
archive stays in the manifest as a compatibility fallback. Large archives are
...
`huggingface_hub`, computes SHA256 for every snapshot file, creates plain
`snapshot.tar.gz` fallback archives, splits each archive into per-model chunks,
uploads the individual files and archive chunks to R2, then uploads
`manifest.json`.
